...
# Misc.
TamaGoはGTPの独自拡張としてtamago-read_sgfコマンドをサポートします。これはGTP標準のloadsgfコマンドと似ていますが、SGFファイルのパスではなくSGF文字列そのものを引数として、次の例のように使います。loadsgfとは異なりmove_numberの指定はできません。また、SGF文字列は改行を含んではいけません。
